...
# Live mode can enable GFPGAN after startup; avoid crashing the preview thread if the model is missing.
_ENHANCER_LOAD_FAILED: bool = False
...
def enhance_face(temp_frame: Frame) -> Frame:
    ...
    global _ENHANCER_LOAD_FAILED
    if _ENHANCER_LOAD_FAILED:
        return temp_frame
    try:
        session = get_face_enhancer()
    except (FileNotFoundError, RuntimeError) as e:
        _ENHANCER_LOAD_FAILED = True
        update_status(
            "GFPGAN disabled: place gfpgan-1024.onnx in the models folder "
            f"(see Hugging Face deep-live-cam). ({e})",
            NAME,
        )
        return temp_frame
