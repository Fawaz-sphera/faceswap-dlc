...
def swap_face(source_face: Face, target_face: Face, temp_frame: Frame) -> Frame:
    ...
    remote_url = os.environ.get("DLC_REMOTE_SWAP_URL", "").strip().rstrip("/")
    ...
    try:
        ...
        if remote_url:
            input_size = int(os.environ.get("DLC_REMOTE_SWAP_INPUT_SIZE", "128"))
            aimg, M = face_align.norm_crop2(temp_frame, target_face.kps, input_size)
            from modules.remote_swap_client import remote_swap_aligned

            bgr_fake = remote_swap_aligned(aimg, source_face.normed_embedding)
        ...
        if bgr_fake is None:
            ...
            if face_swapper is None:
                update_status(
                    "Face swapper model not loaded or failed to load. Skipping swap.",
                    NAME,
                )
